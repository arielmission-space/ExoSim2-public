...
class AddGainDrift(Task):
    ...
    def model(
        ...
        subexposures : Signal
            Sub-exposures cached signal.
        parameters : dict
            Dictionary containing the detector parameters for gain noise computation.
        ...
        if "gain_drift_amplitude" in parameters["detector"]:
            amplitude = parameters["detector"]["gain_drift_amplitude"]
        elif "gain_drift_amplitude_range_min" in parameters["detector"]:
            amplitude_min = parameters["detector"]["gain_drift_amplitude_range_min"]
            amplitude_max = parameters["detector"]["gain_drift_amplitude_range_max"]
            amplitude = RunConfig.random_generator.uniform(amplitude_min, amplitude_max)
            if out_grp is not None:
    ...
    @staticmethod
    def _noise_generator(f: np.ndarray, psd: np.ndarray, out=None) -> np.ndarray:
        """
        ...
        """
        # Randomize amplitude
        noise = RunConfig.random_generator.normal(0, psd * np.sqrt(f[-1]), psd.size)
        if out is not None:
        ...
        return np.fft.irfft(noisef) * np.sqrt(2 * noisef.size - 1)
