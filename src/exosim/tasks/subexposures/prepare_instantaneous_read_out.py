...
from exosim.tasks.subexposures.compute_reading_scheme import ComputeReadingScheme
...
class PrepareInstantaneousReadOut(Task):
    ...
    It calls  :class:`~exosim.tasks.subexposures.compute_reading_scheme.ComputeReadingScheme` to compute the ramp sampling scheme,
    and :class:`~exosim.tasks.subexposures.estimate_ch_jitter.EstimateChJitter` to scale the input pointing jitter to the focal planet pixel units.
    ...
    def __init__(self):
        ...
        pointing_jitter: (:class:`~astropy.units.Quantity`, :class:`~astropy.units.Quantity`,  :class:`~astropy.units.Quantity`)
            Tuple containing the pointing jitter in the spatial and spectral direction expressed in units of deg, and jitter time expressed as sec.
        parameters: dict
            dictionary containing the channel parameters.
            This is usually parsed from :class:`~exosim.tasks.load.load_options.LoadOptions`
    ...
    def execute(self):
        ...
        if pointing_jitter != (None, None, None):
            ...
            readout_freq = check_units(parameters["readout"]["readout_frequency"], "Hz")
        ...
        focal_time_sampled = (focal_plane.time * focal_plane.time_units).to(u.s)
        ...
        if pointing_jitter != (None, None, None):
            ...
            try:
                jitter_rms = min(jitter_rms_array[np.nonzero(jitter_rms_array)])
            except ValueError:
                self.debug("jitter rms is zero in both directions")
                jitter_rms = 0
            self.debug(f"jitter rms: {jitter_rms:.2f}")

            # check if specific resolution is required else set it to 3 by default
            try:
                jitter_res = parameters["detector"]["jitter_rms_min_resolution"]
            ...
            if forced_mag:
                if forced_mag < mag:
                    self.warning(
                        f"suggested jitter magnification ({forced_mag}) is too small: {mag} used instead"
                    )
                else:
                    mag = forced_mag
    ...
    def _average_pointing(
        jitter: np.ndarray, start: np.ndarray, stop: np.ndarray
    ) -> np.ndarray:
        jitter = jitter.astype(np.float64)
        start = start.astype(np.int64)
        stop = stop.astype(np.int64)

        out = np.empty(start.shape[0], dtype=np.float64)
