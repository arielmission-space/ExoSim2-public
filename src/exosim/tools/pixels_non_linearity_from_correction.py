...
class PixelsNonLinearityFromCorrection(PixelsNonLinearity):
    ...
    def compute_coefficients(
        ...
        keys = [
            k for k in parameters["detector"] if "pnl_coeff" in k and "std" not in k
        ]
        if not keys:
        ...
        self.debug(f"correction coefficients: {coeff[::-1]}")
        ...
        while idx == [1023]:
            ...
            oper = parameters["detector"].get("pnl_correction_operator", "/")
            Q = self.operator_dict[oper](
                Qdet, poli(Qdet)
            )  # linear detector pixel count in adu

            pnl_coeff, _pcov = curve_fit(poli_model, Q, Qdet / Q)
            idx = np.argmin(np.abs(poli_model(Q, *pnl_coeff) - 0.95))
            self.debug(f"fitted coefficients: {pnl_coeff}")
            self.debug(f"Well depth: {Q[idx]:.0f} (adu)")
    ...
    def _plot(self, poli, Qmax, pnl_coeff, poli_model, oper) -> None:
        Qdet = np.linspace(1, Qmax * 1.2, self.Npt)  # detector pixel counts in adu
        ...
        ax.axvline(
            Qmax,
            c="b",
            label=f"nominal saturation: {int(np.round(Qmax))}",
            ls="--",
            alpha=0.5,
        )
        ax.axvline(
            Q[idx],
            c="k",
            label=f"real saturation (5% from linear): {int(np.round(Q[idx]))}",
            ls=":",
            alpha=0.5,
        )
