...
class LoadPsf(Task):
    ...
    def execute(self):
        ...
        if output_file and issubclass(output_file.__class__, output.Output):
            ...
            # Write psf_cube if it doesn't exist
            try:
                ...
                if "name already exists" in str(e):
                    self.debug("PSF cube dataset already exists, skipping write")
                else:
                    raise e

            # Write norm if it doesn't exist
            try:
                output_group.write_array("norm", norms)
            except ValueError as e:
                if "name already exists" in str(e):
                    self.debug("Norm dataset already exists, skipping write")
                else:
                    raise e
            ...
            try:
                ...
                if "name already exists" in str(e):
                    self.debug("Wavelength dataset already exists, skipping write")
                else:
                    raise e
