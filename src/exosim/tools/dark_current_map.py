...
class DarkCurrentMap(Task):
    ...
    def execute(self):
        ...
        self.debug(f"dark current map: {dc_map.data}")
    ...
    def model(self, parameters, time):
        ...
        t = [0] * u.hr
        # temporal variation
        if "dc_aging_factor" in parameters["detector"]:
        ...
        dc_map = signal.Dimensionless(
            data=resized_map,
            spectral=np.arange(0, parameters["detector"]["spectral_pix"]) * u.pix,
            time=t,
        )
    ...
    def compute_dc_mean(self, detector):
        ...
        ----------
        detector: dict
            Dictionary for the detector. This is usually parsed from :class:`~exosim.tasks.load.load_options.LoadOptions`
        ...
        def root(s):
            return detector["dc_sigma"].value ** 2 - (np.exp(s**2) - 1) * np.exp(
                2 * mu + s**2
            )
        ...
        if not np.isclose(root(sigma), 0.0, atol=atol):
            self.error(f"dc_sigma root not close to 0 (tol={atol:.1e})")
            raise ValueError(f"dc_sigma root not close to 0 (tol={atol:.1e})")
