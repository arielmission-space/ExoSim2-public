...
class ReadoutSchemeCalculator(ExoSimTool):
    ...
    def __init__(self, options_file, input_file):
        ...
        for ch in self.ch_list:
            ...
            self.info(f"Suggested readout scheme for {ch}")
            (
                exposure_time,
                n_clk_GND,
                n_clk_ndr,
                ...
                n_GRPs,
                n_clk_RST,
            ) = self._compute_scheme(focal_plane, frg_focal_plane, self.ch_param[ch])
            self._print_results(
                exposure_time,
                n_clk_GND,
                n_clk_ndr,
    ...
    def _compute_scheme(self, focal_plane, frg_focal_plane, parameters):
        ...
        ndr_freq = check_units(parameters["readout"]["readout_frequency"], u.Hz)
        ...
        if "exposure_time" in parameters["readout"]:
            exposure_time = parameters["readout"]["exposure_time"]
            exposure_time = check_units(exposure_time, "s")
        else:
            exposure_time = saturation_time

        self.debug(
            f"number of simulation clock in saturation time: {exposure_time * clock}"
        )
        ...
        if exposure_time > saturation_time:
            self.warning(
                f"Saturation problem: exposure time ({exposure_time}) is {exposure_time / saturation_time * 100:.1f}% greater than saturation time ({saturation_time})."
            )
            if "exposure_time" not in parameters["readout"]:
                while exposure_time > saturation_time:
    ...
    def _print_results(
        ...
        self.info("------------------------------------")
        self.info(f"n_NRDs_per_group:         {n_NRDs_per_group}")
        self.info(f"n_GRPs:                   {n_GRPs}")
        self.info(f"n_sim_clocks_Ground:      {n_clk_GND}")
        self.info(f"n_sim_clocks_first_NDR:   {n_clk_ndr}")
        self.info(f"n_sim_clocks_Reset:       {n_clk_RST}")
        self.info(f"n_sim_clocks_groups:      {n_clk_GRP}")
        self.info("------------------------------------")
