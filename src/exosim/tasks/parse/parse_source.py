...
class ParseSources(Task):
    """
    Given the source description, it parses the sources elements and return a dictionary.
    It also applies the time variation if provided.
    ...
    >>>
    >>> plt.plot(
    ...     source_out["HD 209458"].spectral,
    ...     source_out["HD 209458"].data[0, 0],
    ... )
...
class ParseSource(Task):
    ...
    def _parse_sed(self, parameters, wl):
        ...
        if "source_task" in parameters:
            # if custom source is created by the user skip the automatic part
            source_task = find_task(parameters["source_task"], CreateCustomSource)
        ...
        for k in k_list:
            ...
            if k in list(parameters.keys()):
                ...
                self.debug(f"source {k} found in parameters: {parameters[k]}")
        ...
        if "online_database" in list(parameters.keys()):
            ...
            kwards["R"] = star["Radius"]["value"] * u.Unit(star["Radius"]["unit"])
