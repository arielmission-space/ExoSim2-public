...
class FindAstronomicalSignals(Task):
    ...
    def execute(self) -> None:
        ...
        self.signals_dict = {}
        if "source" not in self.sky_parameters:
            self.set_output(self.signals_dict)
            return
        if isinstance(self.sky_parameters["source"], OrderedDict):
            for source in self.sky_parameters["source"]:
                for effect in self.sky_parameters["source"][source]:
                    if isinstance(self.sky_parameters["source"][source][effect], dict):
                        self.find_signals(
            ...
            for effect in self.sky_parameters["source"]:
        ...
        if len(self.signals_dict) > 1:
            ...
            raise ValueError(
                "Conflicting astronomical signals detected across multiple sources"
            )
        ...
        if "signal_task" in input_dict:
            ...
            signal_dict = {
                ...
                "parsed_parameters": (
                    ...
                    if source in self.sky_parameters["source"]
                    else self.sky_parameters["source"]
                ),
            }
            if source not in self.signals_dict:
