...
class ForegroundsToFocalPlane(Task):
    ...
    def execute(self):
        ...
        for rad in [k for k in path if "radiance" in k]:
            if sub_focal_plane:
                tmp_focal_plane = sub_focal_plane.copy()
                tmp_focal_plane.dataset_name = f"sub_focal_planes/{rad}"

            if "slit_width" in path[rad].metadata:
    ...
    def _look_for_isolate(path):
        ...
        return any(not rad.split("_")[-1].isdigit() for rad in rad_list)
