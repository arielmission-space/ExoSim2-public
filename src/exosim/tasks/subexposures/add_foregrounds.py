...
class AddForegrounds(Task):
    ...
    def __init__(self):
        ...
        self.add_task_param("subexposures", "sub-exposures cached signal class")
        self.add_task_param(
            "frg_focal_plane",
            "channel foreground focal plane",
        )
    ...
    def execute(self):
        ...
        self.debug(f"time indexes: {index}")

        # decimate the frg to the subexposure size (take every osf-th pixel)
        frg = frg[:, ::osf, ::osf]
