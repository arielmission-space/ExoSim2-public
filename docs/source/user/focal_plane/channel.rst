...
Recalling the road to the production of the focal plane presented in :ref:`Focal plane creation`,
after the parsing of sources, the foregrounds and the common optics, we have to move inside each channel.

.. image:: _static/road_to_focal_plane.png
    :width: 600
    :align: center

The channels are described in the `.xml` as
...
Where the output use and keyword are to be used only if needed.

Optical path
------------
The first action to take with a channel is to parse the dedicated optical path, as already shown in :ref:`channel optical path`.
...
The following script is to be appended to the previous one and assumes that all the preceding scripts have already been run:

.. code-block:: python

        channel.parse_path(light_path=common_path)

This will parse the channel-specific optical path and attach it after the common optical path already parsed and stored in `common_path`.
The :meth:`~exosim.models.channel.Channel.parse_path` method populates the `path` attribute.
The resulting transmission and radiances are datacubes of the size of the :ref:`wavelength grid` and the :ref:`temporal grid`, encapsulated in :class:`~exosim.models.signal.Signal` classes.
To learn more about the :class:`~exosim.models.signal.Signal` class, refer to :ref:`signal`. This format allows for wavelength- and time-dependent contributions.
...
The default task is :class:`~exosim.tasks.instrument.load_responsivity.LoadResponsivity`.
...
The result is expressed in :math:`\text{counts/Joule}` and returned as a datacube over the :ref:`wavelength grid` and the :ref:`temporal grid`, encapsulated in a :class:`~exosim.models.signal.Signal`.
...
.. caution::
    ...
    will automatically use the default task: :class:`~exosim.tasks.instrument.load_responsivity.LoadResponsivity`.


Propagate foreground
---------------------

To propagate the foregrounds means to multiply the results of the optical path by the detector responsivity and the correct solid angle.
...
The pixel surface, :math:`A_{pix}`, is computed from the detector section of the `.xml` configuration file:
...
The solid angle, :math:`\Omega_{pix}`, as already discussed in :ref:`supported optical elements`, depends on the position of the optical element with respect to the detector.
Everything that comes from the pixel field of view is multiplied by the solid angle subtended by an elliptical aperture on-axis.
The algorithm used is from Equation 56 of John T. Conway, *Nuclear Instruments and Methods in Physics Research Section A: Accelerators, Spectrometers, Detectors and Associated Equipment*, 614(1), 17–27, 2010 (https://doi.org/10.1016/j.nima.2009.11.075).
To estimate :math:`\Omega_{pix}` from this equation, other information is required, such as the f-numbers in the two directions:
...
If the light comes from the optics box, it is then multiplied by :math:`\pi - \Omega_{pix}`. If it comes from the back of the detector, the `detector box`, then it is multiplied by :math:`\pi`.
...
Similarly to what was seen before, to propagate the source means multiplying the source SED by the instrument efficiency to get the density signal expressed as :math:`counts / s / \mu m`.
