...
class ComputeSaturation(Task):
    ...
    def execute(self):
        ...
        well_depth = check_units(well_depth, desired_units=u.ct, calling_class=self)
        ...
        if frg_focal_plane is not None:
            if isinstance(frg_focal_plane, Signal | CountsPerSecond):
                frg_focal_plane = frg_focal_plane.data * frg_focal_plane.data_units
            else:
        ...
        self.debug(f"well depth filled in {saturation_time}")
        f_well_depth = f_well_depth if f_well_depth is not None else 1
        saturation_time *= f_well_depth
        self.debug(f"saturation time : {saturation_time}")

        self.set_output([saturation_time, max_signal, min_signal])
