...
class InstantaneousReadOut(Task):
    ...
    def execute(self):
        ...
        start_index = readout_parameters["ndr_start_cumulative_sequence"].astype(int)
        end_index = readout_parameters["ndr_end_cumulative_sequence"].astype(int)
        ...
        if pointing_jitter != (None, None, None):
            ...
            if saveMemory:
                ...
                if mag != 1:
                    self.info(f"resampling the focal plane: magnification factor {mag}")
                    ...
                    self.debug(f"focal plane resampled: new shape {focal.shape}")
                self.info(
                    "jittering {} for {}".format(dataset_name, parameters["value"])
                )
            ...
            if parameters.get("force_power_conservation"):
            ...
            for chunk in iterate_over_chunks(
                out.dataset,
                desc="applying integration time {}".format(parameters["value"]),
            ):
            ...
            self.info(
                "no jitter in {} for {}".format(dataset_name, parameters["value"])
            )
    ...
    @staticmethod
    def get_oversample_factors(fp, ad_osf):
        """
    ...
    def oversample(fp: np.array, ad_osf: int) -> np.array:
        ...
        """
        xin, yin, xout, yout = InstantaneousReadOut.get_oversample_factors(fp, ad_osf)
    ...
    def jittering_the_focalplane_by_slice(
        ...
        for y in range(time_line_slice.shape[0]):
            ...
            for x in range(time_line_slice.shape[1]):
                ...
                for idx in range(start_index[ndr], end_index[ndr]):
                    ...
                    # if negative index, then roll the array
                    time_line_slice[y, x] = time_line_slice[y, x] + fp[j_jit, i_jit]
        ...
        return time_line_slice / (end_index[ndr] - start_index[ndr])
    ...
    def jittering_the_focalplane(
        ...
        for ndr in prange(start_index.shape[0]):
            ...
            for y in range(time_line.shape[1]):
                ...
                j = j + osf
            # divide by the number of jitter positions added
            time_line[ndr] = time_line[ndr] / (end_index[ndr] - start_index[ndr])
        return time_line

    @staticmethod
    @jit(nopython=True, parallel=True)
    def replicating_the_focalplane(
