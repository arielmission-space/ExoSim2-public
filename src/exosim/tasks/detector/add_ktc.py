...
class AddKTC(Task):
    ...
    def execute(self):
        ...
        for chunk in iterate_over_chunks(subexposures.dataset, desc="adding ktc noise"):
    ...
    def add_offset(
        ...
        state_machine = state_machine.astype(np.int64)
        ...
        for i in range(1, dset.shape[0]):
            if state_machine[i] == state_machine[i - 1]:
                ...
                offset = np.random.normal(0.0, bias_std, dset[i].shape).astype(
                    np.float64
                )
                dset[i] += offset
        return dset, offset
