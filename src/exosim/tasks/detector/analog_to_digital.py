...
class AnalogToDigital(Task):
    ...
    def execute(self):
        ...
        round_method = "floor"
        if "ADC_round_method" in parameters["detector"]:
        ...
        if "ADC_num_bit" in parameters["detector"]:
            bits_num = parameters["detector"]["ADC_num_bit"]
            if isinstance(bits_num, float) and not bits_num.is_integer():
                self.error("number of ADC bits should be integer")
                raise TypeError("number of ADC bits should be integer")
            self.debug(
                "ADC set to {} bits.".format(parameters["detector"]["ADC_num_bit"])
            )
        else:
            bits_num = 32
        ...
        if bits_num > 32:
            self.error(f"ADC bit depth {bits_num} exceeds maximum allowed (32)")
            raise ValueError(f"ADC bit depth {bits_num} exceeds maximum allowed (32)")
        ...
        self.info(f"ADC gain:{gain_factor}")
        self.info(f"ADC offset:{offset}")
        self.info(f"ADC range: {dtype_range}")
