...
class ApplyAstronomicalSignal(Task):
    ...
    def execute(self):
        ...
        se_time = subexposures.time * subexposures.time_units + integration_times / 2
        self.debug(f"se mid-times: {se_time}")
        ...
        self.debug(f"model start: {start_t}. model end: {end_t}")
        ...
        for chunk in iterate_over_chunks(
            ...
            if chunk[0] is not None:
                ...
                subexposures.dataset[chunk] *= multiplicative_signal[:, np.newaxis, :]
    ...
    def select_chunk_range(self, chunk: slice, start_t: int, end_t: int) -> slice:
        """
