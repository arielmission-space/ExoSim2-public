...
class LoadPhoenix(Task):
    ...
    >>> sed = loadPhoenix(
    ...     path=phoenix_directory, T=T, D=D, R=R, z=z, logg=logg
    ... )
    ...
    def execute(self):
        ...
        if filename:  # if filename is given it ignores anything else and it uses that
            ...
            if path:
                ...
                fits_file_name = self.get_phoenix_model_filename(path, T, logg, z)
            else:
        ...
        self.debug(f"phoenix sed loaded: {sed.data}")

        self.set_output(sed)

    def load(self, ph_file):
        """
        Returns a sed given a Phoenix filename
        Parameters
        ----------
        ph_file: str
            filename
        ...
        return signal.Sed(spectral=wl_ph, data=sed_ph)
    ...
    def get_phoenix_model_filename(self, path, T, logg, z):
        ...
        sed_T_list = np.array([float(os.path.basename(k)[3:8]) for k in sed_name])
        sed_Logg_list = np.array([float(os.path.basename(k)[9:12]) for k in sed_name])
        sed_Z_list = np.array([float(os.path.basename(k)[13:16]) for k in sed_name])
        ...
        self.debug(f"phoenix file selected: {ph_file}")
        return ph_file
