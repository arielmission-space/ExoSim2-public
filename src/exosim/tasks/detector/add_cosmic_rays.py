...
class AddCosmicRays(Task):
    ...
    def model(
        ...
        pixel_size = check_units(pixel_size, "cm", force=False)
        randomise = parameters["detector"].get("cosmic_rays_randomise", False)
        ...
        for chunk in iterate_over_chunks(
            ...
            for t in range(data.shape[0]):
                ...
                for x, y in zip(x_pix, y_pix, strict=False):
                    ...
                    shape_str = ""
                    for i, j in zip(shape[0], shape[1], strict=False):
                        shape_str += f"({i + x}, {j + y})"
                        try:
    ...
    def count_events(
        ...
        """
        Calculate the number of cosmic ray events in each sub-exposure, randomly distributing them
        while keeping the probability proportional to the exposure time.
        ...
        randomise : bool
            A flag to randomize the total number of events
        ...
        self.info(f"cosmic rays rate: {scaled_rate}")

        # Compute the total expected number of events
        total_integration_time = np.sum(integration_times)
        self.debug(f"integration times: {integration_times}")

        total_events = (scaled_rate * total_integration_time * saturation_rate).value

        if randomise:
            self.debug("Randomizing total number of events.")
            # Randomize total event
            total_events = RunConfig.random_generator.poisson(total_events)

        self.info(f"cosmic total events: {total_events}")

        # Normalize integration times to create a probability distribution
        integration_weights = integration_times / total_integration_time
        probabilities = integration_weights  # Convert to simple float array

        # Generate a random multinomial distribution of events across sub-exposures
        events_counter = np.random.multinomial(int(total_events), probabilities)

        self.debug(f"Probabilities: {probabilities}")
        self.debug(f"Probabilities sum: {np.sum(probabilities)}")
        self.debug(f"Events counter before multinomial: {int(total_events)}")
    ...
    def shapes_and_probs(self, parameters: dict) -> tuple[list, list]:
        ...
        Examples
        --------
        >>> shapes, probs = AddCosmicRays.shapes_and_probs(
        ...     {
        ...         "interaction_shapes": {
        ...             "single": 0.5,
        ...             "line_h": 0.3,
        ...
        if "interaction_shapes" in parameters:
            ...
            if remaining_prob != parameters["interaction_shapes"].get("single", 0):
                self.warning(
