...
class PopulateFocalPlane(Task):
    ...
    def execute(self):
        ...
        # load the psf
        psf = self.load_psf(parameters, focal_plane, output) if psf is None else psf

        i0_, j0_ = locate_wavelength_windows(psf, focal_plane, parameters)

        if sources:
            for source in sources:
                self.info(f"populating focal plane with {source}")
    ...
    def load_psf(parameters, focal_plane, output):
        ...
        """
        if "psf_task" in parameters["psf"]:
            psf_task = find_task(parameters["psf"]["psf_task"], instrument.LoadPsf)
        ...
        # create the psf
            ...
            nzero = parameters["psf"].get("nzero", 4)
            if "size" in parameters["psf"]:
                array_size = [
            ...
            if output is not None:
                ...
                # Write wavelength if it doesn't exist
                try:
...
def populate(
    ...
    for k in range(len(i0)):
        ...
        # psf starting index
        i_start, j_start = 0, 0
        # check that the expected start is not out of the focal plane
        if i0[k] < 0:
            i_start = -i0[k]
            i0[k] = 0
        if j0[k] < 0:
