...
class ApplyIntraPixelResponseFunction(Task):
    ...
    def __init__(self):
        ...
        self.add_task_param("convolution_method", "convolution method", "fftconvolve")
    ...
    def execute(self):
        ...
        self.info(f"applying pixel response function to {focal_plane.dataset_name}")
        ...
        if output_file and issubclass(output_file.__class__, output.Output):
            ...
            # Write irf_kernel if it doesn't exist
            try:
                output_group.write_array("irf_kernel", irf_kernel)
            except ValueError as e:
                if "name already exists" in str(e):
                    pass  # Skip if dataset already exists
                else:
                    raise e

            # Write irf_kernel_delta if it doesn't exist
            try:
        ...
        Parallel(n_jobs=RunConfig.n_job, require="sharedmem")(
            delayed(self._apply_to_time_slice)(focal_plane, t, convolution_func, kwargs)
            for t in tqdm(
                range(focal_plane.time.size),
                total=focal_plane.time.size,
                desc=f"applying {convolution_method}",
            )
        )
    ...
    def select_convoltion_func(self, method, irf, irf_delta, img_delta):
        ...
        if method == "fftconvolve":
            ...
            raise ValueError(f"{method} convolution method not supported")
        self.debug(f"convolution method: {method}. Selected function is {func}")
        self.debug(f"keywords: {kwargs}")
        return func, kwargs
    ...
    def _apply_to_time_slice(self, focal_plane, t, convolution_func, kwargs):
        """
        Applies convolution to a single time slice using padding to reduce border effects.
        """
        focal_plane.data[t] = convolution_func(focal_plane.data[t], **kwargs)
