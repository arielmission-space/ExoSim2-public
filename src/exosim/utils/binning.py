...
def rebin(
    ...
    mode: str (optional)
        the mode indicates the statistic to use for binning by :func:`scipy.stats.binned_statistic`.
        Default is 'mean'.
    ...
    x, xp, fp = _clean_up_inputs(x, xp, fp, axis, logger=logger)
    ...
    try:
        if (
            ...
            return _bin_with_statistics(x, xp, fp, axis, statistic=mode, logger=logger)
        return _interpolating(x, xp, fp, axis, fill_value=fill_value, logger=logger)
    except ValueError:
...
def _bin_with_statistics(x, xp, fp, axis, statistic="mean", logger=None):
...
def _interpolating(x, xp, fp, axis, fill_value=0.0, logger=None):
...
def _clean_up_inputs(x, xp, fp, axis, logger=None):
