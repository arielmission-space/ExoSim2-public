...
import exosim.tasks.instrument.load_psf_paos as load_psf_paos


class LoadPsfPaosTimeInterp(load_psf_paos.LoadPsfPaos):
    """
    ...
    def model(self, filename, parameters, wavelength, time):
        ...
        # re-normalize output
        return self.normalise(psf_out, psf_out_cropped)
    ...
    def normalise(psf_out, psf_out_cropped):
        ...
        return psf_out_cropped_norm_ * old_norms_reshaped
    ...
    def load_psfs(self, filename, wavelength, parameters):
        ...
        for file in filename:
            with h5py.File(os.path.expanduser(file), "r") as data:
                ...
                psf_cube = [
                    self.load_imac(wl, data, parameters, oversampling, delta_pix)
            ...
            order = np.argsort(wl_unsorted)
            wl_sorted = wl_unsorted[order]
            psf_cube_interp = self.wl_interpolate(
                psf_cube, wl_sorted, wl_sampled.to(u.um)
            )
            # reorder the array back to the original wavelength ordering
            inv_order = np.argsort(order)
            psf_cube_interp = psf_cube_interp[inv_order, ...]
