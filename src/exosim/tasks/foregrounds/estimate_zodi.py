...
class EstimateZodi(Task):
    ...
    >>> wavelength = (
    ...     np.logspace(np.log10(0.45), np.log10(2.2), 6000) * u.um
    ... )
    >>> zodi = estimateZodi(
    ...     wavelength=wavelength, zodiacal_factor=1
    ... )
    ...
    def __init__(self):
        ...
        self.add_task_param("zodiacal_factor", "zodiacal multiplicative factor", None)
    ...
    def execute(self):
        ...
        if coordinates:
            zodiacal_factor = self.zodiacal_fit_direction(coordinates, zodi_map)

        rad = self.model(zodiacal_factor, wl)
        self.debug(f"zodical radiance: {rad.data}")
