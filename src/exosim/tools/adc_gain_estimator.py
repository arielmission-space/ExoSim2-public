...
class ADCGainEstimator(ExoSimTool):
    ...
    def __init__(self, options_file, output=None):
        ...
        for ch in self.ch_list:
            gain_factor, dtype_max_val, int_type = self.model(self.ch_param[ch])

            self.info(f"---------- {ch} --------")
            self.info(f"gain factor: {gain_factor}")
            self.info(f"max adc value: {dtype_max_val}")
            self.info(f"integer dtype: {int_type}")
    ...
    def model(self, parameters):
        ...
        if "ADC_max_value" in parameters["detector"]:
