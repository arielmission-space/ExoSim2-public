...
class LoadWavelengthSolution(Task):
    ...
    def execute(self):
        ...
        for k in ["wavelength", "spectral", "spatial"]:
            if k not in wl_solution.colnames:
                self.error(f"missing {k} keyword")
                raise KeyError(f"missing {k} keyword")
            if wl_solution[k].unit != u.um:
                self.error(f"wrong units: {k} units are not micron")
                raise u.UnitsError(f"wrong units: {k} units are not micron")
    ...
    def model(self, parameters):
        ...
        """
        wl_solution = QTable()
        wl_solution["wavelength"] = parameters["wl_solution"]["data"]["Wavelength"]
