...
class ParsePath(Task):
    ...
    Note
    ------
    The user can force the parser to isolate contribution by adding to the description dictionary the key 'isolate' set to ``True``.
    ...
    def __init__(self):
        ...
        self.add_task_param("group_name", "group name in output", "contributes")
    ...
    def execute(self):
        ...
        if isinstance(parameters["opticalElement"], OrderedDict):
            for element in parameters["opticalElement"]:
        ...
        # output preparation
        if output_file and issubclass(output_file.__class__, output.Output):
            if "final contributes" in output_file._entry:
    ...
    def _add_new_element(self, parameters, output_file):
        ...
        if "type" in parameters and parameters["type"].lower() == "slit":
            self._parse_slit(parameters)
        # if solid angle
        elif "solid_angle" in parameters:
            self._parse_angle(parameters, output_file)
        # if isolate, store it in its own way
        elif parameters.get("isolate"):
        ...
        # otherwise it updates it
            ...
            if not self.radiance_key.split("_")[-1].isdigit():
                self._parse_new_radiance()
            if "slit_width" in self.out[self.radiance_key].metadata:
                self._parse_new_radiance()
            # parse the optical element to get radiance and efficiency
            parsed_out = self._parse_optical_element(parameters, output_file)
            # updates efficiency
    ...
    def _parse_slit(self, parameters):
        ...
        for rad in [x for x in self.out if "radiance" in x]:
    ...
    def _parse_angle(self, parameters, output_file):
        ...
        self.out[self.radiance_key] = parsed_out["radiance"]
        # it append the solid angle to the metadata of the previous radiance
        self.out[self.radiance_key].metadata["solid_angle"] = parameters["solid_angle"]

    def _parse_optical_element(self, parameters, output_file):
        # Handle nested parameters structure (e.g., {'default': {...}})
        if "value" not in parameters and len(parameters) == 1:
            # If there's a single key that's not 'value', use that nested dict
            nested_key = next(iter(parameters.keys()))
            parameters = parameters[nested_key]

        if "value" not in parameters:
            self.error(
                f"Missing 'value' key in optical element parameters: {parameters}"
            )
            raise KeyError("'value' key is required in optical element parameters")
    ...
    def radiance_keys_list(self):
        ...
        return [
            int(dat_re.search(rad).group()) for rad in self.out if dat_re.search(rad)
        ]
    ...
    def _update_radiance_key(self, name=None):
        ...
        if self.radiance_keys_list:
            ...
            i = 0
        if name:
            self.radiance_key = f"radiance_{i}_{name}"
        else:
            self.radiance_key = f"radiance_{i}"

        self.debug(f"output radiance key updated to {self.radiance_key}")
    ...
    def _parse_new_radiance(self):
        ...
        radiance = signal.Radiance(
            spectral=self.wl,
            data=np.zeros((self.tt.size, 1, self.wl.size)) * u.W / u.m**2 / u.um / u.sr,
            time=self.tt,
        )
        # prepare output dict
