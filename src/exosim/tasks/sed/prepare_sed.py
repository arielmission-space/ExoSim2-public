...
from .create_planck_star import CreatePlanckStar
from .download_sed import DownloadSed
from .load_custom import LoadCustom
from .load_phoenix import LoadPhoenix
...
class PrepareSed(Task):
    ...
    def __init__(self):
        ...
        model_name: str (optional)
            sed model name to download, for a list of available models see
            :func:`~exosim.tasks.sed.download_sed.get_svo_models`. Default is `bt-settl-cifist`.
        """
        ...
        self.add_task_param("model_name", "sed model name", "bt-settl-cifist")
    ...
    def execute(self):
        ...
        if source_type == "planck":
            ...
            sed = downloadSed(T=T, R=R, D=D, logg=logg, z=z, model_name=model_name)
