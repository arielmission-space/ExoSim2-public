...
class NDRsPlotter(log.Logger):
    ...
    def plot_ndrs(
        ...
        ax0.set_title(f"{ch} - num {i} - time {t_str}")

        im = ax0.imshow(ndrs[i], interpolation="none")
        plt.colorbar(im, ax=ax0)

        fname = os.path.join(out_dir, f"{ch}_ndrs_{i}.png")
        fig.savefig(fname, format="png", dpi=300)

        self.info(f"plot saved in {i}")

    def load_ndrs(self, ch: str, f: h5py.File) -> tuple[np.ndarray, u.Quantity, int]:
        """
        ...
        self.debug(f"NDRs loaded from {ch}")
