...
class PixelsNonLinearity(ExoSimTool):
    ...
    def __init__(
        ...
        for ch in self.ch_list:
            out_dict = {}
            self.info(f"computing pixel no linearity coefficients for {ch}")
            pnl_coeff, sat = self.compute_coefficients(self.ch_param[ch], show_results)
    ...
    def compute_coefficients(
        ...
        pnl_coeff = np.array(
            [(-1) ** i * 1 / factorial(i + 1) * constant**i for i in range(1, 5)]
        )
    ...
    def _plot(self, saturation, pnl_coeff) -> None:
        Q = np.linspace(1, saturation * 1.2, 2**10)  # detector pixel counts in adu
        ...
        ax.axvline(
            saturation,
            c="k",
            label=f"real saturation (5% from linear): {int(np.ceil(saturation))}",
            ls="-",
        )
    ...
    def _print_results(self, coeff: list[float], sat: float) -> None:
        ...
        self.info(f"saturation (5% from linear): {int(sat)} (counts)")

        self.info("------------------------------------")
        self.info("pnl_coeff_a:     1")
        self.info(f"pnl_coeff_b:     {coeff[0]}")
        self.info(f"pnl_coeff_c:     {coeff[1]}")
        self.info(f"pnl_coeff_d:     {coeff[2]}")
        self.info(f"pnl_coeff_e:     {coeff[3]}")
        self.info("------------------------------------")
    ...
    def create_map(
        ...
        std = parameters["detector"].get("pnl_coeff_std", 0.0)
