...
from .utils import _create_ordered_cmap, find_channels_position, prepare_channels_list
...
class FocalPlanePlotter(log.Logger):
    ...
    >>> focalPlanePlotter = FocalPlanePlotter(
    ...     input="./test_file.h5"
    ... )
    ...
    def _prepare_figure(self) -> tuple[plt.Figure, GridSpec, list[str]]:
        with h5py.File(self.input, "r") as f:
            channels_path = (
                find_channels_position(self.input)
            ...
            widths = []
            for ch in ch_list:
                size_x = channels[ch]["focal_plane"]["spectral"].shape[0]
                size_y = channels[ch]["focal_plane"]["spatial"].shape[0]
    ...
    def load_focal_plane(self, ch: str, time_step: int) -> tuple[np.ndarray, int]:
        """
        ...
        with h5py.File(self.input, "r") as f:
            ...
            frg = load_signal(f[os.path.join(file_path, "frg_focal_plane")]).data[
                time_step
            ]
            try:
                bkg = load_signal(f[os.path.join(file_path, "bkg_focal_plane")]).data[
                    time_step
                ]
            except KeyError:
                bkg = 0
        ...
        self.debug(f"focal planes loaded from {ch}")

        return final, osf

    def plot_focal_plane(self, time_step: int = 0, scale="linear") -> plt.Figure:
        """
    ...
    def plot_efficiency(
        ...
        efficiency: str = "all",
    ) -> tuple[plt.Figure, list[mpl.axes.Axes]]:
        """
        ...
        if efficiency not in valid:
            self.error(
                f"invalid efficiency option '{efficiency}'. "
        ...
        def bands(axs):
            try:
                for ax in axs.flatten():
                    self.plot_bands(ax, scale, channel_edges, add_legend=ch_lengend)
            except Exception:
        ...
        with h5py.File(self.input, "r") as f:
            if efficiency == "optical efficiency":
                ...
                for k, ch in enumerate(channels):
                    eff_path = f"{channels_path}/{ch}/efficiency"
                ...
                bands(axs)
                for k, ch in enumerate(channels):
                    resp_path = f"{channels_path}/{ch}/responsivity"
                ...
                axs.set_ylabel(rf"${resp.data_units}$")
                axs.set_xlabel(r"Wavelength [$\mu m$]")
                axs.grid()
