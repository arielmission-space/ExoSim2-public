...
class EstimateSpectralBinning(Task):
    ...
    def execute(self):
        ...
        for key in key_list:
            if key not in table.colnames:
                self.error(f"missing keyword: {key}")
    ...
    def model(self, wl_grid, parameters):
        ...
        if parameters["type"].lower() == "spectrometer":
            ...
            self.error("unsupported channel type: {}", format(parameters["type"]))
    ...
    def wavelength_table_spectrometer(self, description, wl_grid):
        ...
        table["right_bin_edge"] = table["wavelength"] + 0.5 * table["bandwidth"]
        self.debug(f"wavelength table: \n{table}")
        return table
