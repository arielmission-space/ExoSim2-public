...
class ComputeReadingScheme(Task):
    ...
    def __init__(self):
        ...
        self.add_task_param("readout_oversampling", "readout oversampling factor", 1)
    ...
    def execute(self):
        ...
        self.info(f"total observing time: {total_observing_time}")

        clock_freq = check_units(parameters["readout"]["readout_frequency"], u.Hz)
        ...
        n_clk_GND = parameters["readout"]["n_sim_clocks_Ground"] * readout_oversampling
        n_clk_NDR0 = (
            parameters["readout"]["n_sim_clocks_first_NDR"] * readout_oversampling
        )
        n_clk_NDR = parameters["readout"].get("n_sim_clocks_NDR", 1)
        n_clk_NDR *= readout_oversampling
        n_clk_RST = parameters["readout"]["n_sim_clocks_Reset"] * readout_oversampling
        n_clk_GRP = parameters["readout"]["n_sim_clocks_groups"] * readout_oversampling
        ...
        self.info(f"exposure time: {exposure_time}")

        # define number of exposures
        if "n_exposures" in parameters["readout"]:
            number_of_exposures = parameters["readout"]["n_exposures"]
            self.debug(f"number of exposures manually set to {number_of_exposures}")
        else:
            number_of_exposures = np.ceil(
                total_observing_time.to(u.s) / exposure_time
            ).astype(int)
            self.debug(
                f"number of exposures automatically set to {number_of_exposures}"
            )
        self.info(f"number of exposures: {number_of_exposures}")
        ...
        self.info(f"number of NDRs: {number_of_exposures * base_mask.sum()}")

        frame_sequence = np.tile(base, number_of_exposures)
        self.set_output([clock, base_mask, frame_sequence, number_of_exposures])
