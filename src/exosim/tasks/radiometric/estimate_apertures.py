...
class EstimateApertures(Task):
    ...
    def execute(self):
        ...
        for col in required_columns:
            if col not in new_tab.colnames:
    ...
    def model(self, table, focal_plane, description, wl_grid):
        ...
        # spectral settings
        if "spectral_mode" in description:
            if description["spectral_mode"] == "row":
                ...
                if wl_grid is None:
                    self.error("wavelength grid required for wavelength solution mode")
                    raise OSError(
                        "wavelength grid required for wavelength solution mode"
                    )
                ...
                self.error("Unsupported spectral mode")
                raise OSError("Unsupported spectral mode")

        # spatial settings
        if "spatial_mode" in description:
            if description["spatial_mode"] == "column":
                ...
                self.error("Unsupported spatial mode")
                raise OSError("Unsupported spatial mode")

        # automatic settings
        if "auto_mode" in description:
            ...
            if description["auto_mode"] == "elliptical":
                ene = description["EnE"]
                sizes, _surf, ene = aperture.find_elliptical_aperture(focal_plane, ene)
                wlc = [focal_plane.shape[1] / 2] * table["wavelength"].size
            ...
            if description["auto_mode"] == "rectangular":
                ene = description["EnE"]
                sizes, _surf, ene = aperture.find_rectangular_aperture(focal_plane, ene)
                wlc = [focal_plane.shape[1] / 2] * table["wavelength"].size
            ...
            if description["auto_mode"] == "bin":
                ...
                if wl_grid is None:
                    self.error("wavelength grid required for bin mode")
                    raise OSError("wavelength grid required for bin mode")
                ...
                for i in range(wlc.size):
                    size, _surf, ene = aperture.find_bin_aperture(
                        focal_plane,
                        ene,
                        spatial_with=dwl[i],
                        center=(wlc[i], spc[i]),
        ...
        # Calcolo area superficie per ogni bin
        if shape == "rectangular":
            aperture_size = np.array(dwl) * np.array(dsp)
        elif shape == "elliptical":
            aperture_size = np.pi * (np.array(dwl) / 2) * (np.array(dsp) / 2)
        else:
            aperture_size = np.array(dsp)  # fallback generico
        ...
        new_tab["aperture_size"] = aperture_size
