...
The code can be either used as a stand-alone tool and run pre-made pipelines called `recipes`,
or used as a library to build custom pipelines.
...
:code:`exosim-sub-exposures`  creates the sub-exposures from the focal plane (:ref:`sub-exposures creation`)
:code:`exosim-nrds`           creates the NDRs from the sub-exposures (:ref:`ndrs creation`)
============================  ======================================================================================
...
Here are listed the main command-line flags.
...
``-l``, ``--log``             stores the log output in a file
============================  =======================================================================

The configuration file shall be an `.xml` file and the output file an `.h5` file (see later in :ref:`h5`).
...
The main output product is an HDF5_ `.h5` file.
...
Replace the path in the `main_example.xml` file with the path to the `examples` folder on your computer.
...
The first step is to build the focal plane (see :ref:`Focal plane creation`). You can do this by
...
This will produce two plots: the first focal plane and the instrument efficiency versus wavelength:
...
And, again, we can investigate the content by producing useful plots
