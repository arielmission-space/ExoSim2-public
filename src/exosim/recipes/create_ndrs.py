...
class CreateNDRs(TimedClass, log.Logger):
    ...
    def __init__(
        self, input_file: str, output_file: str, options_file: str | dict
    ) -> None:
        """
        Parameters
        ----------
        ...
        with output.use(append=True, cache=True) as out:
            ...
            for ch in ch_list:
                ...
                # prepare output
                self.announce(f"iterating over {ch}")
                ...
                # add dark current
                if look_for_key(ch_description["detector"], "dark_current", True):
                ...
                # add shot noise
                if look_for_key(ch_description["detector"], "shot_noise", True):
                    addShotNoise = detector.AddShotNoise()
                    addShotNoise(subexposures=sub_ndrs, output=sim_grp)

                # add cosmic rays
                if look_for_key(ch_description["detector"], "cosmic_rays", True):
                ...
                # add reset bias
                if look_for_key(ch_description["detector"], "ktc_offset", True):
                ...
                if look_for_key(ch_description["detector"], "dead_pixels", True):
                ...
                if look_for_key(
                    ...
                    pnl_apply_instance(subexposures=sub_ndrs, parameters=pnl_map)

                # apply pixel saturation
                if look_for_key(ch_description["detector"], "saturation", True):
                    sat_instance = find_and_run_task(
                        ch_description["detector"],
                        "sat_task",
                        detector.ApplySimpleSaturation,
                    )
                    sat_instance(subexposures=sub_ndrs, parameters=ch_description)

                # add read noise
                if look_for_key(ch_description["detector"], "read_noise", True):
                ...
                # add gain drift
                if look_for_key(ch_description["detector"], "gain_drift", True):
    ...
    def load_subexposure_data(
        ...
        with h5py.File(self.input, "r") as f:
            ...
            spectral = se["spectral"] * u.Unit(se["spectral_units"][()].decode("utf-8"))
            spatial = se["spatial"] * u.Unit(se["spatial_units"][()].decode("utf-8"))
            time_line = se["time"] * u.Unit(se["time_units"][()].decode("utf-8"))
            integration_times = se["metadata"]["integration_times"]["value"][
                ()
            ] * u.Unit(se["metadata"]["integration_times"]["unit"][()].decode("utf-8"))
            params = dict(f[os.path.join(file_path, "instantaneous_readout_params")])
            try:
    ...
    def prepare_output(
        ...
        with h5py.File(self.input, "r") as f:
            ...
            for key in f[inst_rdo_data]:
                if key in keys_to_keep:
                    f.copy(os.path.join(inst_rdo_data, key), sim_grp._entry)
            if "pointing_jitter" in f:
                for key in f["pointing_jitter"]:
                    f.copy(os.path.join("pointing_jitter", key), sim_grp._entry)
    ...
    def clean_output_tree(self, out: HDF5OutputGroup, key_list: list[str]) -> None:
        """
    ...
    def refactor_output(self, output_file: str) -> None:
        ...
        new_name = f"{filename}_unrefactored{file_extension}"
        ...
        with h5py.File(new_name, "r") as f, h5py.File(output_file, "w") as g:
            copy_file(f, g)
