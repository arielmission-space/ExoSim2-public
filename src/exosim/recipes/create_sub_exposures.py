...
class CreateSubExposures(TimedClass, log.Logger):
        ...
        with output.use(append=True, cache=True) as out:
            ...
            if "jitter" in self.mainConfig:
                ...
                (
                    ...
                    self.jitter_spe,
                    self.jitter_time,
                ) = jitter_instance(parameters=self.mainConfig, output_file=out)

                if "slicing" in self.mainConfig["jitter"]:
            ...
            for ch in ch_list:
                self.announce(f"computing sub-exposure for {ch}")
                ...
                sky_params = self.mainConfig.get("sky", {})
                astrosignals = findAstronomicalSignals(
                    sky_parameters=sky_params,
                )
                for source, signals in astrosignals.items():
                    ...
                    for signal, params in signals.items():
                        ...
                        self.info(f"{source}-{signal} astronomical signal found.")
                        ...
                        timeline = (
                            (out_se.time * out_se.time_units + integration_time / 2)
                            if timeline is None
                            else timeline
                        )
                ...
                if "add_background_to_se" in ch_param["detector"]:
                ...
                try:
                    ...
                    self.warning("No quantum efficiency variation map detected")
    ...
    def load_focal_plane(self, ch: str) -> tuple[CountsPerSecond, CountsPerSecond]:
        """
        ...
        with h5py.File(self.input, "r") as f:
            ...
            try:
                bkg_fp = load_signal(f[os.path.join(file_path, "bkg_focal_plane")])
            except KeyError:
                bkg_fp = None
    ...
    def load_source(self, ch: str, source: str) -> tuple[CountsPerSecond]:
        ...
        self.debug(f"{source} source loaded from {ch}")

        return source

    def copy_simulation_data(self, ch: str, ch_grp: HDF5OutputGroup) -> HDF5OutputGroup:
        """
        ...
        with h5py.File(self.input, "r") as f:
            ...
            for info in ["efficiency", "responsivity"]:
                try:
                    ...
                    self.debug(f"{info} data copied to output")
                except KeyError:
                    continue
        return sim_grp
